...
def test_run_vlm_measure_sums_phase_trace_energy_for_total_energy(monkeypatch, tmp_path):
    ...
    assert payload["summary"]["prefill_energy_j"]["mean"] == pytest.approx(4.0)
    assert payload["summary"]["decode_energy_j"]["mean"] == pytest.approx(6.0)
    ...
    assert payload["summary"]["prefill_tps_per_w"]["mean"] == pytest.approx(32 / 4.0)
    # decode_tokens = num_decode * batch = 2 * 4 = 8; decode_energy_j = 6.0
    assert payload["summary"]["decode_tps_per_w"]["mean"] == pytest.approx(8 / 6.0)
    assert payload["summary"]["prefill_j_per_tok"]["mean"] == pytest.approx(4.0 / 32)
    assert payload["summary"]["decode_j_per_tok"]["mean"] == pytest.approx(6.0 / 8)
