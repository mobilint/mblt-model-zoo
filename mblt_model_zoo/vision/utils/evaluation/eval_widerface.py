...
def eval_widerface(
    model: MBLT_Engine,
    widerface_eval_cfg: dict,
    batch_size: int = 1,
):
    """Evaluate the model on the widerface dataset

    Args:
        model (Model Class): Model class object
        helper (Helper Class): Helper class object with pre_process and post_process methods
        widerface_eval_cfg (dict): Configuration dictionary for the evaluation
    ...
    dataset, dataloader = load_dataloader(
        ...
        dataset_path=widerface_eval_cfg["dataset_path"],
    ...
    for input_npu, org_shape, target_classes, fnames in pbar:
        cum_num_data += len(target_classes)
        tic = time()
        out_npu = model(input_npu)
        inference_time += time() - tic
        nms_outs = model.postprocess(out_npu)
        infer_post_time += time() - tic
        _, boxes_list, scores_list = model._postprocess.nmsout2eval(
        ...
        for i, (boxes, scores) in enumerate(zip(boxes_list, scores_list)):
            results.extend(
                [
                    {
                        "image_id": fnames[i].replace(".jpg", ""),
                        "category_id": target_classes[i],
                        "bbox": boxes,
                        "score": scores,
                    }
                ]
    ...
    pred_boxes = {class_id: {} for class_id in dataset.classes}
    for result in results:
        pred_boxes[result["category_id"]][result["image_id"]] = np.concatenate(
            (
                np.array(result["bbox"], dtype=np.float32).reshape(-1, 4),
                np.array(result["score"], dtype=np.float32).reshape(-1, 1),
            ),
            axis=1,
        )

    pred_boxes = norm_score(pred_boxes)
    aps = evaluation(pred_boxes, widerface_eval_cfg["annotation_path"])
    return (
        aps,
        inference_time,
